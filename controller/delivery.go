...
var deliveryService = service.DeliveryService{} // サービスの実体を作る。

// 納品データ作成
func CSVDeliveryHandler(c *gin.Context) {
	...
	// 投げる
	countingList, err := deliveryService.GetDeliveryData(csvUtilCustomers)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"srvResCode": 500,
			"error":      "数取りに失敗しました",
